...
import { ArrowLeft, DollarSign, Wallet, Percent, ShieldPlus, TrendingUp, Heart, Package, Users, LineChart, ArrowUp, ArrowDown } from "lucide-react";
import { ResponsiveContainer, LineChart as LineChart$1, CartesianGrid, XAxis, YAxis, Tooltip, Line } from "recharts";
import { R as Route, s as supabase, B as Button, S as Select, a as SelectTrigger, b as SelectValue, c as SelectContent, d as SelectItem, I as Input } from "./router-CvBcNStb.js";
...
function AgentDashboardPage() {
  ...
  useEffect(() => {
    let active = true;
    setLoading(true);
    Promise.all([supabase.from("sales").select("*").eq("agent_id", agentId).gte("sale_date", range.from.toISOString()).lte("sale_date", range.to.toISOString()).order("sale_date", {
      ascending: false
    }), supabase.from("sales").select("*").eq("agent_id", agentId).gte("sale_date", prevRange.from.toISOString()).lt("sale_date", prevRange.to.toISOString()), fetchExpensesInRange(range.from, range.to, {
      agentId
    }), fetchExpensesInRange(prevRange.from, prevRange.to, {
      agentId
  ...
  useEffect(() => {
    ...
    const run = async () => {
      const agentRes = await supabase.from("targets").select(cols).eq("scope", "agent").eq("agent_id", agentId).maybeSingle();
  ...
  const topByKind = useMemo(() => {
    ...
    const top = (mm) => [...mm.entries()].map(([name, v]) => ({
      name,
      ...v
  ...
  return /* @__PURE__ */ jsxs("div", { className: "space-y-6", children: [
    /* @__PURE__ */ jsx("div", { className: "flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center", children: /* @__PURE__ */ jsxs("div", { children: [
      /* @__PURE__ */ jsx(Button, { asChild: true, variant: "ghost", size: "sm", className: "-ml-2 mb-2", children: /* @__PURE__ */ jsxs(Link, { to: "/agents", children: [
        /* @__PURE__ */ jsx(ArrowLeft, { className: "mr-1 h-4 w-4" }),
        " All agents"
      ] }) }),
      /* @__PURE__ */ jsx("h1", { className: "text-2xl font-bold tracking-tight sm:text-3xl", children: agentInfo.name }),
    ...
    /* @__PURE__ */ jsxs("div", { className: "surface-card grid grid-cols-1 gap-3 p-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4", children: [
      ...
      /* @__PURE__ */ jsxs("div", { children: [
        ...
        /* @__PURE__ */ jsx(Input, { placeholder: "Sale ID or customer…", value: search, onChange: (e) => setSearch(e.target.value) })
      ] })
    ] }),
